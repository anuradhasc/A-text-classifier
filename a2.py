...
def tokenize(doc, keep_internal_punct=False):
    ...
    if (keep_internal_punct == False):
        ...
        return np.array([d.strip(string.punctuation) for d in doc.lower().split()])
...
def token_pair_features(tokens, feats, k=3):
    ...
    for i in range(len(tokens)-(k)):
        ...
        c=0
        while(c <= k):
            sb.append(tokens[i+c])
            c += 1
        
        #sb = [tokens[i], tokens[i+1], tokens[i+2]]
        #print(sb)
        for j in range(len(sb)):
            z = j+1
            while(z <= len(sb)-1):
                key = "token_pair="+sb[j]+"__"+sb[z]
                #print(key)
                if key in feats:
                    feats[key] += 1
                else:
...
def featurize(tokens, feature_fns):
    ...
    #pass
    feats = defaultdict(lambda:0)
    #print("feature fns",feature_fns)
    #feature_fns[0](tokens, feats)
    for fn in feature_fns:
        fn(tokens, feats)
        #feature_fns[fn](tokens, feats,3)
        #feature_fns[fn](tokens, feats)
    #print(sorted(feats.items(), key=lambda x:x[0]))
...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    #pass
    i=0
    featuresDict =defaultdict(lambda: 0)
    for t in tokens_list:
        featuresDict[i] = featurize(t,feature_fns)
        i += 1
        
    vocabDict = defaultdict(lambda: 0)
    for f in featuresDict:
        for v in featuresDict[f]:
            key = v[0]
            if(vocabDict[key] != 0):
                vocabDict[key] += 1
            else:
    ...
    finalvocabDict=defaultdict(lambda: 0)
    for key,value in vocabDict.items():
        if(value >= min_freq):
            finalvocabDict[key] = value
    
    finalvocabDict= sorted(finalvocabDict.items())
        ...
        for k in featuresDict:
            for v in featuresDict[k]:
                ...
                if key in vocab:
                    ...
                    rows.append(k)
                    #print(vocab[key])
                    columns.append(vocab[key])
                    #print(v[1])
                    data.append(v[1])
        return  csr_matrix((data, (rows, columns)), dtype='int64'),vocab
    else:
        count=0
        for value in finalvocabDict:
            aux[value[0]] = count
            count+=1
        for k in featuresDict:
            for v in featuresDict[k]:
                #print("k",k,"v",v)
                key = v[0]
                if(vocabDict[key] >= min_freq):
                    rows.append(k)
                    #print(aux[key])
                    columns.append(aux[key])
                    #print(v[1])
                    data.append(v[1])
        return  csr_matrix((data, (rows, columns)), dtype='int64'),aux
...
def plot_sorted_accuracies(results):
    ...
    sortedAccuracies = sorted(accuracies)
    plt.clf()
    plt.plot(sortedAccuracies)
    plt.ylabel('accuracy')
    plt.xlabel('setting')
    #plt.show()
...
def mean_accuracy_per_setting(results):
    ...
    for res in results:
        punct.add(res['punct'])
        freq.add(res['min_freq'])
        funct.add((res['features']))
    ...
    for p in punct:
        ...
        accuracyRes.append((np.mean(accuracy),'punct='+(str(p))))
    ...
    for f in freq:
        ...
        for res in results:
            if res['min_freq'] == f:
                #print(res['min_freq'])
                accuracy.append(res['accuracy'])
        accuracyRes.append((np.mean(accuracy),'min_freq='+(str(f))))
    ...
    for fnc in funct:
        ...
        for res in results:
            ...
            for fc in fnc:
                filenames.append(fc.__name__)
                
        fileName = ' '.join(str(n) for n in filenames)
        accuracyRes.append((np.mean(accuracy),'features='+(str(fileName))))
    
    accuracyRes = sorted(accuracyRes, reverse=True)
    return accuracyRes
...
def fit_best_classifier(docs, labels, best_result):
    ...
    tokens=[]
    for doc in docs:
        toks = tokenize(doc, keep_internal_punct=best_result['punct'])
        tokens.append(toks)
    
    X,y=vectorize(tokens, best_result['features'], min_freq=best_result['min_freq'])
...
def top_coefs(clf, label, n, vocab):
    ...
    #print(clf.coef[0][1])
    if label == 1:
        sortedList = sorted(range(len(clf.coef_[0])), key=lambda x: clf.coef_[0][x], reverse=True)[:n]
    else: 
        sortedList = sorted(range(len(clf.coef_[0])), key=lambda x: clf.coef_[0][x])[:n]
    
    ftcoefList = []
    for value in vocab.items():
        for i in sortedList:
            if value[1] == i:
                #print(value[1])
                if clf.coef_[0][i]<0:
                    ftcoefList.append((value[0],-1.0*clf.coef_[0][i]))
                else:
                    ftcoefList.append((value[0],1.0*clf.coef_[0][i]))
                    
    ftcoefList = sorted(ftcoefList, key=lambda x:x[1], reverse=True)
    return ftcoefList
